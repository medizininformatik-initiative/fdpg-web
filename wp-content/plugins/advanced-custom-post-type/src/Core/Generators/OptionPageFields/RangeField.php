...
class RangeField extends AbstractOptionPageField implements MetaFieldInterface
{
	public function render()
	{
		...
		$field = '<input type="hidden" name="'. esc_attr($this->getIdName()).'_type" value="'.OptionPageMetaBoxFieldModel::RANGE_TYPE.'">';
		$field .= '<div class="acpt-range-wrapper">';
		$field .= '<div class="">';
		$field .= '<input '.$this->required().' id="'.esc_attr($this->getIdName()).'" list="'.esc_attr($this->getIdName()).'_markers" name="'. esc_attr($this->getIdName()).'" type="range" class="acpt-form-control acpt-range" value="'. esc_attr($defaultValue).'"';
