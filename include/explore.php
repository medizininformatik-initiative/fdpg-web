...
function explore( $var, $_options = array() ) {
	...
	if ( is_array( $_options ) ) {
		...
		$_options = $explore_default_options;
	}
...
EOF;
	...
	if ( $_options['enable_js'] ) {
		$html_output .= '<a class="all_links" href="javascript:explore_showAll();">Expand all details</a>
		&nbsp;
		<a class="all_links" href="javascript:explore_hideAll();">Collapse all</a>
		<br /><br />';
	}

	$html_output .= explore_var2table( $var, $_options );

	if ( $_options['enable_js'] ) {
		$html_output .= $explore_scripts;

		if ( $_options['expand_all'] ) {
			$html_output .= '<script type="text/javascript">explore_showAll();</script>';
		}
	}
...
function explore_json_url( $json_url, $_options = array() ) {
	$json_file_str = file_get_contents( $json_url );
	if ( ! isset( $_options['title'] ) ) {
		$_options['title'] = '';
		// $_options['title'] = 'Exploring JSON document: ' . $json_url;
	}
	return explore_json( $json_file_str, $_options );
}
...
function explore_var2table( $mixed_var, $_options = array() ) {
	if ( empty( $mixed_var ) ) {
		return null;
	}
	...
	$show_hide = ( $_options['enable_js'] && isset( $_options['deep'] ) && $_options['deep'] );
	...
	foreach ( $mixed_var as $key => $value ) {
		if ( is_array( $value ) || is_object( $value ) ) {
			$inner = explore_var2table( $value, array_merge( $_options, array( 'deep' => true ) ) );
		} else {
			$inner = $value;
		}
		...
		if ( $_options['show_type'] ) {
			$type      = strtolower( gettype( $value ) );
			$type_html = "<td class='type' valign='top'>$type</td>";
		} else {
			$type_html = null;
		}
