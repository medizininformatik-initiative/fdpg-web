...
class LengthField extends AbstractFlexibleField implements FlexibleFieldInterface
{
    public function render()
    {
        ...
        $field = '<input type="hidden" name="'. esc_attr($this->getIdName()).'[type]" value="'.AbstractMetaBoxFieldModel::LENGTH_TYPE.'">';
    ...
    private function getDefaultLengthValue()
    {
        ...
        return (isset($data[$key]) and isset($data[$key][$this->elementIndex]) and isset($data[$key][$this->elementIndex]['length']) ) ? $data[$key][$this->elementIndex]['length'] : 'METER';
    }
}