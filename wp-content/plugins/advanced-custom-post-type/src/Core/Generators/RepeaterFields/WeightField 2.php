...
class WeightField extends AbstractRepeaterField implements RepeaterFieldInterface
{
    public function render()
    {
        ...
        $field = '<input type="hidden" name="'. esc_attr($this->getIdName()).'[type]" value="'.CustomPostTypeMetaBoxFieldModel::WEIGHT_TYPE.'">';
    ...
    private function getDefaultWeightValue()
    {
        ...
        return (isset($data[$key]) and isset($data[$key][$this->index]) and isset($data[$key][$this->index]['weight']) ) ? $data[$key][$this->index]['weight'] : 'KILOGRAM';
    }
}