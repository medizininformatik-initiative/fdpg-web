...
class AWP_Simvoly extends appfactory
{
    ...
    public function init_actions(){
        ...
        add_action('admin_post_awp_simvoly_save_api_token', [$this, 'awp_save_simvoly_api_token'], 10, 0);
    ...
    public function settings_view($current_tab)
    {
        ...
        <div class="wrap">
                ...
                $platform_obj = new AWP_Platform_Shell_Table('simvoly');
    ...
    public function awp_save_simvoly_api_token()
    {
        if (!current_user_can('administrator')) {
            die(esc_html__('You are not allowed to save changes!', 'automate_hub'));
        }
        // Security Check
        if (!wp_verify_nonce($_POST['_nonce'], 'awp_simvoly_settings')) {
            die(esc_html__('Security check Failed', 'automate_hub'));
        }

        $api_token = sanitize_text_field($_POST["awp_simvoly_api_token"]);
        $display_name     = sanitize_text_field( $_POST["awp_simvoly_display_name"] );
        $url     = sanitize_text_field( $_POST["awp_simvoly_domain_url"] );

        if ($url != "") {

            $url_regexPattern = '/^https:\/\/.[a-zA-Z0-9][a-zA-Z0-9\-]*\.simvoly\.com\/\z/';
            if (preg_match($url_regexPattern, $url)) {
                // Save tokens
                $platform_obj = new AWP_Platform_Shell_Table('simvoly');
                $platform_obj->save_platform(['account_name'=>$display_name,'api_key' => $api_token, 'url' => $url ]);

                AWP_redirect("admin.php?page=automate_hub&tab=simvoly");
            } else {
                wp_safe_redirect(admin_url('admin.php?page=automate_hub&tab=simvoly'));
                exit();
            }
        } else {
            wp_safe_redirect(admin_url('admin.php?page=automate_hub&tab=simvoly'));
    ...
    public function load_custom_script() {
        wp_enqueue_script( 'awp-simvoly-script', AWP_URL . '/apps/s/simvoly/simvoly.js', array( 'awp-vuejs' ), '', 1 );
    }

    public function action_fields() {
        ?>
            <script type="text/template" id="simvoly-action-template">
                <?php

                    $app_data=array(
                            'app_slug'=>'simvoly',
                           'app_name'=>'Simvoly ',
                           'app_icon_url'=>AWP_ASSETS.'/images/icons/simvoly.png',
                           'app_icon_alter_text'=>'Simvoly  Icon',
    ...
    public function awp_fetch_source() {
        ...
        $url = "https://api.simvoly.com/v1/sources";
        $args = array(
            'headers' => array(
...
function awp_simvoly_save_integration() {
    Appfactory::save_integration();
}

function awp_simvoly_send_data( $record, $posted_data ) {
...
function awp_simvoly_resend_data( $log_id,$data,$integration ) {
