    ...
    public function render()
    {
	    ...
	    if($this->isABlockElement()){
		    ...
		    if($data !== null and isset($data['value'])){
			    return $this->addBeforeAndAfter('<a href="tel:'.$data['value'].'" target="_blank">'.$data['value'].'</a>');
		    }
        ...
        return $this->addBeforeAndAfter('<a href="tel:'.$this->fetchMeta($this->getKey()).'" target="_blank">'.$this->fetchMeta($this->getKey()).'</a>');
    }
}