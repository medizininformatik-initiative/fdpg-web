...
class GalleryField extends AbstractRepeaterField implements RepeaterFieldInterface
{
    public function render()
    {
        ...
        $defaultValue = ( $this->getDefaultValue() !== '' ) ? esc_attr(implode(',', $this->getDefaultValue())) : '';

        $field = '<input type="hidden" name="'. esc_attr($this->getIdName()).'[type]" value="'.CustomPostTypeMetaBoxFieldModel::GALLERY_TYPE.'">';
	    ...
	    $field .= '<input id="'.$id.'_id" type="hidden" name="'. esc_attr($this->getIdName()).'[id]" value="' .implode(',', $attachmentIds).'">';
        ...
        if(is_array($this->getDefaultValue())){
            foreach ($this->getDefaultValue() as $index => $value){
                $field .= '<input name="'. esc_attr($this->getIdName()).'[type][]" data-index="'.$index.'" type="hidden" value="'.CustomPostTypeMetaBoxFieldModel::GALLERY_TYPE.'">';
