...
//			    }
    ...
    public function fetchMetaAction()
    {
        ...
        if(($belongsTo === MetaTypes::CUSTOM_POST_TYPE or $belongsTo === MetaTypes::TAXONOMY) and $find === null){
            return wp_send_json([
                'success' => false,
                'error' => 'No data sent'
            ]);
    ...
    public function fetchLicenseAction()
    {
        ...
        $pluginFetch = ACPTApiClient::call('/plugin/fetch');
                        ...
                        if(isset($field['relations'])){
                            foreach ($field['relations'] as $relationIndex => $relation) {
                                ...
                                $relatedCustomPostType = @CustomPostTypeRepository::get([
                                        'postType' => $relation['relatedEntity']['value']
                                ], true)[0];

                                if($relatedCustomPostType !== null){
							...
							if($inversedField and $inversedField['type'] !== CustomPostTypeMetaBoxFieldModel::POST_TYPE){
								switch ($relation['type']) {
									case Relationships::ONE_TO_ONE_BI:
										$oppositeRelation = Relationships::ONE_TO_ONE_BI;
										break;

									case Relationships::ONE_TO_MANY_BI:
										$oppositeRelation = Relationships::MANY_TO_ONE_BI;
										break;

									case Relationships::MANY_TO_ONE_BI:
										$oppositeRelation = Relationships::ONE_TO_MANY_BI;
										break;

									case Relationships::MANY_TO_MANY_BI:
										$oppositeRelation = Relationships::MANY_TO_MANY_BI;
										break;
								}
