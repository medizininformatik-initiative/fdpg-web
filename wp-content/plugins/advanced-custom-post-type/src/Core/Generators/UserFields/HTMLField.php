...
class HTMLField extends AbstractUserField implements MetaFieldInterface
{
    public function render()
    {
        ...
        $field .= '<textarea '.$this->required().' id="'.esc_attr($this->getIdName()).'" name="'. esc_attr($this->getIdName()).'" class="regular-text acpt-codemirror" rows="8">'.esc_attr($this->getDefaultValue()).'</textarea>';
