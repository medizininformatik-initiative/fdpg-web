...
class OptionPageMetaController extends AbstractController
{
	/**
	 ...
	 * @return mixed
	 */
	public function getByMenuSlug(\WP_REST_Request $request)
	{
		$slug = $request['slug'];
	...
	public function create(\WP_REST_Request $request)
	{
		...
		try {
			...
			if(!empty($meta)){
				return $this->jsonResponse([
					'message' => 'This page already has associated meta boxes'
				], 500);
			}
	...
	{
		// validate data
		$this->validateJSONSchema($datum, new OptionPageMetaBoxSchema());
		...
		$metaBoxModel = OptionPageMetaBoxModel::hydrateFromArray([
			'id' => $id,
			'optionPage' => $slug,
			'name' => $datum["title"],
			'sort' => ($index+1),
		...
		foreach ($datum["fields"] as $fieldIndex => $field){
			...
			$metaBoxFieldModel = OptionPageMetaBoxFieldModel::hydrateFromArray([
				'id' => $fieldId,
			...
			if($field['type'] === OptionPageMetaBoxFieldModel::REPEATER_TYPE and !empty($field['children'])){

				foreach ($field['children'] as $childIndex => $child){

					$childFieldId = isset($child["id"]) ? $child["id"] : Uuid::v4();

					$metaBoxChildFieldModel = OptionPageMetaBoxFieldModel::hydrateFromArray([
						'id' => $childFieldId,
						'metaBox' => $metaBoxModel,
						'name' => $child['name'],
						'type' => $child['type'],
					...
					if($child['type'] === OptionPageMetaBoxFieldModel::SELECT_TYPE or $child['type'] === OptionPageMetaBoxFieldModel::SELECT_MULTI_TYPE){
			...
			if($field['type'] === OptionPageMetaBoxFieldModel::FLEXIBLE_CONTENT_TYPE and isset($field['blocks'])){
				foreach ($field['blocks'] as $blockIndex => $block){
					...
					if(isset($block['fields'])){
						foreach ($block['fields'] as $nestedFieldIndex => $nestedField){
							...
							$metaBoxNestedFieldModel = OptionPageMetaBoxFieldModel::hydrateFromArray([
								'id' => $nestedFieldId,
								'metaBox' => $metaBoxModel,
							...
							if($nestedField['type'] === OptionPageMetaBoxFieldModel::SELECT_TYPE or $nestedField['type'] === OptionPageMetaBoxFieldModel::SELECT_MULTI_TYPE){
			...
			if($field['type'] === OptionPageMetaBoxFieldModel::SELECT_TYPE or $field['type'] === OptionPageMetaBoxFieldModel::SELECT_MULTI_TYPE){
	...
	public function update(\WP_REST_Request $request)
	{
		...
		try {
			...
			if(empty($meta)){
				return $this->jsonResponse([
					'message' => 'This option page has not associated meta boxes'
				], 500);
			}
