...
class LengthField extends AbstractUserField implements MetaFieldInterface
{
    public function render()
    {
        ...
        $field = '<input type="hidden" name="'. esc_attr($this->getIdName()).'_type" value="'.UserMetaBoxFieldModel::LENGTH_TYPE.'">';
        $field .= '<div class="currency-group">
                    <div class="currency-symbol">'.$list[$this->getDefaultLengthValue()]['symbol'].'</div>
                    <input '.$this->required().' id="'.esc_attr($this->getIdName()).'" name="'. esc_attr($this->getIdName()).'" type="number" min="'.$min.'" max="'.$max.'" step="'.$step.'" class="regular-text currency-amount" value="'
    ...
    {
	    $savedValue = get_user_meta($this->userId, esc_attr($this->getIdName()).'_length', true);
