...
function awp_moonmail_settings_view( $current_tab ) {
	...
	<a href="https://sperse.io/go/moonmail" target="_blank"><img src="<?php echo AWP_ASSETS; ?>/images/logos/moonmail.png" width="232" height="50" alt="MoonMail Logo"></a><br/><br/>
	<?php 
...
$form_fields = '';
$app_name= 'moonmail';
$moonmail_form = new AWP_Form_Fields($app_name);

$form_fields = $moonmail_form->awp_wp_text_input(
    array(
        'id'            => "awp_moonmail_display_name",
        'name'          => "awp_moonmail_display_name",
        'value'         => $display_name,
...
$form_fields .= $moonmail_form->awp_wp_text_input(
    array(
        'id'            => "awp_moonmail_api_key",
        'name'          => "awp_moonmail_api_key",
        'value'         => $api_key,
        'placeholder'   => esc_html__( 'Please enter Account ID', 'automate_hub' ),
        'label'         =>  esc_html__( 'Account ID', 'automate_hub' ),
        'wrapper_class' => 'form-row',
        'show_copy_icon'=>true
...
$form_fields .= $moonmail_form->awp_wp_hidden_input(
    array(
        'name'          => "action",
        'value'         => 'awp_moonmail_save_api_key',
    )
);
...
$moonmail_form->render($form_fields);
...
?>
    ...
    <div class="wrap">
        <form id="form-list" method="post">
            ...
            <?php
            $data=[
                        'table-cols'=>['account_name'=>'Display name','api_key'=>'Account ID','spots'=>'Active Spots','active_status'=>'Active']
                ];
            $platform_obj= new AWP_Platform_Shell_Table('moonmail');
...
add_action( 'admin_post_awp_moonmail_save_api_key', 'awp_save_moonmail_api_key', 10, 0 );

function awp_save_moonmail_api_key() {

      if ( ! current_user_can('administrator') ){
        die( esc_html__( 'You are not allowed to save changes!','automate_hub' ) );
    }
    // Security Check
    if (! wp_verify_nonce( $_POST['_nonce'], 'awp_moonmail_settings' ) ) {
        die( esc_html__( 'Security check Failed', 'automate_hub' ) );
    }

    $api_key = sanitize_text_field( $_POST["awp_moonmail_api_key"] );
    $display_name     = sanitize_text_field( $_POST["awp_moonmail_display_name"] );
    // Save keys
    $platform_obj= new AWP_Platform_Shell_Table('moonmail');
    $platform_obj->save_platform(['account_name'=>$display_name,'api_key'=>$api_key]);
 

    AWP_redirect( "admin.php?page=automate_hub&tab=moonmail" );
}

add_action( 'awp_add_js_fields', 'awp_moonmail_js_fields', 10, 1 );

function awp_moonmail_js_fields( $field_data ) {}
    ...
    <script type="text/template" id="moonmail-action-template">
		        <?php
                    ...
                    $app_data=array(
                            'app_slug'=>'moonmail',
                           'app_name'=>'MoonMail',
                           'app_icon_url'=>AWP_ASSETS.'/images/icons/moonmail.png',
                           'app_icon_alter_text'=>'MoonMail Icon',
...
add_action( 'wp_ajax_awp_get_moonmail_list', 'awp_get_moonmail_list', 10, 0 );
/*
 * Get Kalviyo subscriber lists
 */
function awp_get_moonmail_list() {
 ...
 */
function awp_moonmail_save_integration() {
    $params = array();
 ...
 */
function awp_moonmail_send_data( $record, $posted_data ) {
    ...
    if( $task == "subscribe" ) {
        ...
        $data = array(
            'Address'=>$email,
            'UserAttributes'=>array('name'=>$first_name.$last_name)
        );

        $url = "https://client.moonmail.io/{$api_key}/contacts";
        ...
        $args['headers']['x-api-key']='XXXXXXXXXX';
        $args['body']=$data;
        $url = "https://client.moonmail.io/XXXXXXXXXX/contacts";
...
function awp_moonmail_resend_data( $log_id,$data,$integration ) {
