...
class RangeField extends AbstractRepeaterField implements RepeaterFieldInterface
{
    public function render()
    {
        ...
        $field = '<input type="hidden" name="'. esc_attr($this->getIdName()).'[type]" value="'.CustomPostTypeMetaBoxFieldModel::RANGE_TYPE.'">';
	    ...
	    $field .= '<div class="">';
        $field .= '<input '.$this->required().' id="'.esc_attr($this->getIdName()).'[value]" name="'. esc_attr($this->getIdName()).'[value]" list="'.esc_attr($this->getIdName()).'_markers" type="range" class="acpt-admin-meta-field-input acpt-range" value="'. esc_attr($defaultValue).'"';
