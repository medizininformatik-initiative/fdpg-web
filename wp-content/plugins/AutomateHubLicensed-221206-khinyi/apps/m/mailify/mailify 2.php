...
function awp_mailify_settings_view( $current_tab ) {
	...
	<a href="https://sperse.io/go/mailify" target="_blank"><img src="<?php echo AWP_ASSETS; ?>/images/logos/mailify.png" width="170" height="50" alt="Mailify Logo"></a><br/><br/>
	<?php 
...
$form_fields = '';
$app_name= 'mailify';
$mailify_form = new AWP_Form_Fields($app_name);

$form_fields = $mailify_form->awp_wp_text_input(
    array(
        'id'            => "awp_mailify_display_name",
        'name'          => "awp_mailify_display_name",
        'value'         => $display_name,
...
$form_fields .= $mailify_form->awp_wp_text_input(
    array(
        'id'            => "awp_mailify_account_number",
        'name'          => "awp_mailify_account_number",
        'value'         => $client_id,
        'placeholder'   => esc_html__( 'Enter Account Number', 'automate_hub' ),
        'label'         =>  esc_html__( 'Account Number', 'automate_hub' ),
        'wrapper_class' => 'form-row',
        'show_copy_icon'=>true
...
$form_fields .= $mailify_form->awp_wp_text_input(
    array(
        'id'            => "awp_mailify_key",
        'name'          => "awp_mailify_key",
        'value'         => $api_key,
        'placeholder'   => esc_html__( 'Enter Key', 'automate_hub' ),
        'label'         =>  esc_html__( 'Key', 'automate_hub' ),
        'wrapper_class' => 'form-row',
        'show_copy_icon'=>true
...
$form_fields .= $mailify_form->awp_wp_hidden_input(
    array(
        'name'          => "action",
        'value'         => 'awp_save_mailify_api_key',
    )
);
...
$mailify_form->render($form_fields);
...
?>
    ...
    <div class="wrap">
        <form id="form-list" method="post">
            ...
            <?php
            $data=[
                        'table-cols'=>['account_name'=>'Display name','client_id'=>'Account Number','api_key'=>'API Key','spots'=>'Active Spots','active_status'=>'Active']
                ];
            $platform_obj= new AWP_Platform_Shell_Table('mailify');
...
add_action( 'admin_post_awp_save_mailify_api_key', 'awp_save_mailify_api_key', 10, 0 );

function awp_save_mailify_api_key() {

      if ( ! current_user_can('administrator') ){
        die( esc_html__( 'You are not allowed to save changes!','automate_hub' ) );
    }
    // Security Check
    if (! wp_verify_nonce( $_POST['_nonce'], 'awp_mailify_settings' ) ) {
        die( esc_html__( 'Security check Failed', 'automate_hub' ) );
    }

    $account_number = sanitize_text_field( $_POST["awp_mailify_account_number"] );
    $key            = sanitize_text_field( $_POST["awp_mailify_key"] );
    $display_name     = sanitize_text_field( $_POST["awp_mailify_display_name"] );

    // Save tokens
    $platform_obj= new AWP_Platform_Shell_Table('mailify');
    $platform_obj->save_platform(['account_name'=>$display_name,'api_key'=>$key,'client_id'=>$account_number]);

    AWP_redirect( "admin.php?page=automate_hub&tab=mailify" );
}

add_action( 'awp_add_js_fields', 'awp_mailify_js_fields', 10, 1 );

function awp_mailify_js_fields( $field_data ) {}
    ...
    <script type="text/template" id="mailify-action-template">
		        <?php
                    ...
                    $app_data=array(
                            'app_slug'=>'mailify',
                           'app_name'=>'Mailify',
                           'app_icon_url'=>AWP_ASSETS.'/images/icons/mailify.png',
                           'app_icon_alter_text'=>'Mailify Icon',
                           'account_select_onchange'=>'getMailifyList',
                           'tasks'=>array(
                                        'subscribe'=>array(
...
add_action( 'wp_ajax_awp_get_mailify_list', 'awp_get_mailify_list', 10, 0 );
/*
 * Get Mailify subscriber lists
 */
function awp_get_mailify_list() {
    // Security Check
 ...
 */
function awp_mailify_save_integration() {
    $params = array();
 ...
 */
function awp_mailify_send_data( $record, $posted_data ) {
...
function awp_mailify_resend_data( $log_id,$data,$integration ) {
