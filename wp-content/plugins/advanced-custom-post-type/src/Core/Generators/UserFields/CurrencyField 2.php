...
class CurrencyField extends AbstractUserField implements MetaFieldInterface
{
    public function render()
    {
        ...
        $icon = 'bx:bx-euro';
        $field = '<input type="hidden" name="'. esc_attr($this->getIdName()).'_type" value="'.UserMetaBoxFieldModel::CURRENCY_TYPE.'">';
        $field .= '<div class="currency-group">
                    <div class="currency-symbol">'.Currencies::getList()[$this->getDefaultCurrencyValue()]['symbol'].'</div>
                    <input '.$this->required().' id="'.esc_attr($this->getIdName()).'" name="'. esc_attr($this->getIdName()).'" type="number" min="'.$min.'" max="'.$max.'" step="'.$step.'" class="currency-amount regular-text" value="'.esc_attr($this->getDefaultValue()).'">
    ...
    {
	    $savedValue = get_user_meta($this->userId, esc_attr($this->getIdName()).'_currency', true);
