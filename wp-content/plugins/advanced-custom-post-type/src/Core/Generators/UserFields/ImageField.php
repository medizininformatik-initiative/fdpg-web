...
class ImageField extends AbstractUserField implements MetaFieldInterface
{
    public function render()
    {
        ...
        $icon = 'bx:bx-image';
        $field = '<input type="hidden" name="'. esc_attr($this->getIdName()).'_type" value="'.UserMetaBoxFieldModel::IMAGE_TYPE.'">';
        ...
        $field .= '<input readonly '.$this->required().' id="'.esc_attr($this->getIdName()).'" name="'. esc_attr($this->getIdName()).'" type="text" class="regular-text" value="' .esc_attr($this->getDefaultValue()) .'" style="margin-right: 4px;">';
        $field .= '<a class="upload-image-btn button button-primary" style="margin-right: 4px;">'.Translator::translate("Upload").'</a>';
