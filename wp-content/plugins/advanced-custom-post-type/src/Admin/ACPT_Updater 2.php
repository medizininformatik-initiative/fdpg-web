...
class ACPT_Updater
{
        ...
        if( property_exists( $transient, 'checked') ) {
            if( $checked = $transient->checked ) {
                ...
                if(is_array($pluginInfo) and isset($pluginInfo['version'])){
                    ...
                    if( $outOfDate ) {
                        $newFiles = LicenseApi::downloadLink();
    ...
    public function pluginPopup( $result, $action, $args )
    {
        if( ACPT_License_Manager::isLicenseValid() and ! empty( $args->slug ) ) {
            if( $args->slug == current( explode( '/' , $this->basename ) ) ) {
                ...
                $version = $pluginInfo['version'];
                $version = str_replace(['v.', 'v'], '', $version);
                $downloadLink = LicenseApi::downloadLink();
    ...
    private function getPluginInfo()
    {
        ...
        if(false === $cachedPluginInfo or '' === $cachedPluginInfo){
            $pluginInfo = LicenseApi::call('/plugin/fetch');
            set_transient( $transientKey, $pluginInfo, 3600 );
    ...
    public function sendInsights()
    {
        ...
        if(false === $cachedPluginInfo){
            ...
            $insights = LicenseApi::call('/license/insights/send', [
            ...
            set_transient( $transientKey, $insights, 86400 );
        }
    }
}