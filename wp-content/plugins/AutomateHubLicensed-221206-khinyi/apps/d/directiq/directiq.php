...
function awp_directiq_settings_view( $current_tab ) {
	...
	<a href="https://sperse.io/go/directiq" target="_blank"><img src="<?php echo AWP_ASSETS; ?>/images/logos/directiq.png" width="233" height="50" alt="DirectIQ Logo"></a><br/><br/>
	<?php 
...
$form_fields = '';
$app_name= 'directiq';
$directiq_form = new AWP_Form_Fields($app_name);

$form_fields = $directiq_form->awp_wp_text_input(
    array(
        'id'            => "awp_directiq_display_name",
        'name'          => "awp_directiq_display_name",
        'value'         => $display_name,
...
$form_fields .= $directiq_form->awp_wp_text_input(
    array(
        'id'            => "awp_directiq_api_key",
        'name'          => "awp_directiq_api_key",
        'value'         => $api_key,
...
$form_fields .= $directiq_form->awp_wp_text_input(
    array(
        'id'            => "awp_directiq_api_secret",
        'name'          => "awp_directiq_api_secret",
        'value'         => $api_secret,
        'placeholder'   => esc_html__( 'Enter API Secret', 'automate_hub' ),
        'label'         =>  esc_html__( 'API Secret', 'automate_hub' ),
        'wrapper_class' => 'form-row',
        'show_copy_icon'=>true
...
$form_fields .= $directiq_form->awp_wp_hidden_input(
    array(
        'name'          => "action",
        'value'         => 'awp_save_directiq_api_key',
    )
);
...
$directiq_form->render($form_fields);
...
?>
    ...
    <div class="wrap">
        <form id="form-list" method="post">
            ...
            $platform_obj= new AWP_Platform_Shell_Table('directiq');
...
add_action( 'admin_post_awp_save_directiq_api_key', 'awp_save_directiq_api_key', 10, 0 );

function awp_save_directiq_api_key() {
    ...
    // Security Check
    if (! wp_verify_nonce( $_POST['_nonce'], 'awp_directiq_settings' ) ) {
        die( esc_html__( 'Security check Failed', 'automate_hub' ) );
    }
    $display_name = sanitize_text_field( $_POST["awp_directiq_display_name"] );
    $api_key   = sanitize_text_field( $_POST["awp_directiq_api_key"] );
    $spi_secret   = sanitize_text_field( $_POST["awp_directiq_api_secret"] );
    // Save tokens
    $platform_obj= new AWP_Platform_Shell_Table('directiq');
    $platform_obj->save_platform(['api_key'=>$api_key,'client_secret'=>$spi_secret,'account_name'=>$display_name]);
    AWP_redirect( "admin.php?page=automate_hub&tab=directiq" );
}

add_action( 'awp_add_js_fields', 'awp_directiq_js_fields', 10, 1 );

function awp_directiq_js_fields( $field_data ) {}
    ...
    <script type="text/template" id="directiq-action-template">
		<?php
                    ...
                    $app_data=array(
                            'app_slug'=>'directiq',
                           'app_name'=>'DirectIQ',
                           'app_icon_url'=>AWP_ASSETS.'/images/icons/directiq.png',
                           'app_icon_alter_text'=>'DirectIQ Icon',
                           'account_select_onchange'=>'getDirectiqList',
                           'tasks'=>array(
                                        'subscribe'=>array(
...
add_action( 'wp_ajax_awp_get_directiq_list', 'awp_get_directiq_list', 10, 0 );
/*
 * Get DirectIQ subscriber lists
 */
function awp_get_directiq_list() {
    if ( ! current_user_can('administrator') ){
        die( esc_html__( 'You are not allowed to save changes!','automate_hub' ) );
    }
    ...
    $str= base64_encode($api_key.":".$api_secret);
    $args = array(
        'headers' => array(
    ...
    $url  = "http://rest.directiq.com/contacts/lists/list";
 ...
 */
function awp_directiq_save_integration() {
    $params = array();
 ...
 */
function awp_directiq_send_data( $record, $posted_data ) {
    ...
    if( $task == "subscribe" ) {
        ...
        $url = "http://rest.directiq.com/contacts/lists/importcontacts/{$list_id}";
        $str= base64_encode($api_key.":".$api_secret);
        ...
        $args['headers']['Authorization']='XXXXXXXXXX';
        $args['body']=$data;
...
function awp_directiq_resend_data($log_id,$data,$integration){
