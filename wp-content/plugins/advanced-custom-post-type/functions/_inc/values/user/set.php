...
if( !function_exists('add_acpt_user_meta_field_value') ){
    ...
    function add_acpt_user_meta_field_value(array $args = [])
    {
        try {
            ...
            $saved_field_type = get_user_meta($user_id, $meta_field_model->getDbName().'_type', true);
            if($saved_field_type !== $meta_field_model_type){
                if(!update_user_meta($user_id, $meta_field_model->getDbName().'_type', $meta_field_model_type)){
                    return false;
                }
            }
            ...
            switch ($meta_field_model_type){
                ...
                case UserMetaBoxFieldModel::ADDRESS_TYPE:
                    ...
                    $saved_field_value = get_user_meta($user_id, $meta_field_model->getDbName(), true);

                    if($saved_field_value !== $value){

                        if(!update_user_meta($user_id, $meta_field_model->getDbName(), Sanitizer::sanitizeRawData(UserMetaBoxFieldModel::ADDRESS_TYPE, $value))){
                            return false;
                        }
                        ...
                        if($coordinates){
                            if(!update_user_meta($user_id, $meta_field_model->getDbName().'_lat', Sanitizer::sanitizeRawData(UserMetaBoxFieldModel::TEXT_TYPE, $coordinates['lat']))){
                                return false;
                            }

                            if(!update_user_meta($user_id, $meta_field_model->getDbName().'_lng', Sanitizer::sanitizeRawData(UserMetaBoxFieldModel::TEXT_TYPE, $coordinates['lng']))){
                                return false;
                            }
                        }
                    }
                ...
                case UserMetaBoxFieldModel::CURRENCY_TYPE:
                    ...
                    if($saved_field_value !== $amount){
                        ...
                        if(!update_user_meta($user_id, $meta_field_model->getDbName(), Sanitizer::sanitizeRawData(UserMetaBoxFieldModel::CURRENCY_TYPE, $amount))){
                            return false;
                        }

                        if(!update_user_meta($user_id, $meta_field_model->getDbName().'_currency', Sanitizer::sanitizeRawData(UserMetaBoxFieldModel::TEXT_TYPE, $unit))){
                            return false;
                        }
                    }
	            ...
	            case UserMetaBoxFieldModel::DATE_RANGE_TYPE:
		            ...
		            $value = implode(" - ", $value);
		            if(!update_user_meta($user_id, $meta_field_model->getDbName(), Sanitizer::sanitizeRawData($meta_field_model_type, $value))){
			            return false;
		            }
                ...
                case UserMetaBoxFieldModel::FILE_TYPE:
	                ...
	                $saved_field_value = get_user_meta($user_id, $meta_field_model->getDbName(), true);
	                $saved_field_label = get_user_meta($user_id, $meta_field_model->getDbName().'_label', true);
	                $saved_file_id = get_user_meta($user_id, $meta_field_model->getDbName().'_id', true);

	                if($idFile !== $saved_file_id){
		                if(!update_user_meta($user_id, $meta_field_model->getDbName().'_id', $idFile)){
			                return false;
		                }
	                }

	                if($saved_field_value !== $url){
		                if(!update_user_meta($user_id, $meta_field_model->getDbName(), Sanitizer::sanitizeRawData($meta_field_model_type, $url))){
			                return false;
		                }
	                }

	                if($saved_field_label !== $label){
		                if(!update_user_meta($user_id, $meta_field_model->getDbName().'_label', Sanitizer::sanitizeRawData(UserMetaBoxFieldModel::TEXT_TYPE, $label))){
			                return false;
		                }
	                }
                ...
                case UserMetaBoxFieldModel::GALLERY_TYPE:
	                ...
	                $saved_file_ids = get_user_meta($user_id, $meta_field_model->getDbName().'_id', true);

	                if($saved_file_ids !== $idFiles){
		                if(!update_user_meta($user_id, $meta_field_model->getDbName().'_id', $idFiles)){
			                return false;
		                }
	                }
                ...
                case UserMetaBoxFieldModel::LENGTH_TYPE:
                    ...
                    if($saved_field_value !== $length){
                        ...
                        if(!update_user_meta($user_id, $meta_field_model->getDbName(), Sanitizer::sanitizeRawData(UserMetaBoxFieldModel::WEIGHT_TYPE, $length))){
                            return false;
                        }

                        if(!update_user_meta($user_id, $meta_field_model->getDbName().'_length', Sanitizer::sanitizeRawData(UserMetaBoxFieldModel::TEXT_TYPE, $unit))){
                            return false;
                        }
                    }
	            ...
	            case UserMetaBoxFieldModel::NUMBER_TYPE:
	            case UserMetaBoxFieldModel::RANGE_TYPE:
                ...
                case UserMetaBoxFieldModel::LIST_TYPE:
                    ...
                    foreach ($value as $item){
                        if(!add_acpt_user_meta_field_row_value([
                            'user_id' => $user_id,
                ...
                case UserMetaBoxFieldModel::WEIGHT_TYPE:
                    ...
                    if($saved_field_value !== $weight){
                        ...
                        if(!update_user_meta($user_id, $meta_field_model->getDbName(), Sanitizer::sanitizeRawData(UserMetaBoxFieldModel::WEIGHT_TYPE, $weight))){
                            return false;
                        }

                        if(!update_user_meta($user_id, $meta_field_model->getDbName().'_weight', Sanitizer::sanitizeRawData(UserMetaBoxFieldModel::TEXT_TYPE, $unit))){
                            return false;
                        }
                    }
	            ...
	            // RATING_TYPE
	            case UserMetaBoxFieldModel::RATING_TYPE:
		            ...
		            $saved_field_value = get_user_meta($user_id, $meta_field_model->getDbName(), true);
		            if((int)$saved_field_value !== $value){
			            if(!update_user_meta($user_id, $meta_field_model->getDbName(), Sanitizer::sanitizeRawData($meta_field_model_type, $value))){
				            return false;
			            }
		            }
        ...
        try {
            ...
            switch ($meta_field_model_type){
                ...
                case UserMetaBoxFieldModel::LIST_TYPE:
                    ...
                    if(!update_user_meta($user_id, $meta_field_model->getDbName(), Sanitizer::sanitizeRawData(UserMetaBoxFieldModel::LIST_TYPE, $values))){
                        return false;
                    }
...
if( !function_exists('delete_acpt_user_meta_field_row_value') ){
    ...
    function delete_acpt_user_meta_field_row_value(array $args = [])
    {
        try {
            ...
            if(!update_user_meta($user_id, $meta_field_model->getDbName(), Sanitizer::sanitizeRawData(UserMetaBoxFieldModel::LIST_TYPE, array_values($saved_field_value)))){
                return false;
            }
            ...
            switch ($meta_field_model_type){
                ...
                case UserMetaBoxFieldModel::LIST_TYPE:
                    ...
                    if($saved_field_value[$index] !== $value){
                        ...
                        if(!update_user_meta($user_id, $meta_field_model->getDbName(), Sanitizer::sanitizeRawData(UserMetaBoxFieldModel::LIST_TYPE, $saved_field_value))){
                            return false;
                        }
                    }
