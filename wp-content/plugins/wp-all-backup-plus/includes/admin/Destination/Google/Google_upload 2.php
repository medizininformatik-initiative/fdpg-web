...
add_action('wp_all_backup_completed', array('WPALLBackupGoogle', 'wp_all_backup_completed'));

class WPALLBackupGoogle {

    public static function wp_all_backup_completed(&$args) {

        $authCode = get_option('wpall_dest_google_authCode');
        $clientId = get_option('wpall_dest_google_client_key');
        $clientSecret = get_option('wpall_dest_google_secret_key');
        ...
        if (!empty($authCode) && !empty($clientId) && !empty($clientSecret)) {
            ...
            $file->setDescription("WP All Backup : backup file-" . site_url());
            ...
            $args[2] = $args[2] . '<br> Upload Backup on google drive';
             $args[6] = $args[6] .="Drive, ";
