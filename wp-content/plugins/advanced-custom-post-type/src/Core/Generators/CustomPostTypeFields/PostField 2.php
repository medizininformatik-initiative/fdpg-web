...
class PostField extends AbstractCustomPostTypeField implements MetaFieldInterface
{
    ...
    public function render()
    {
        ...
        $options = $this->getOptions($relation['related_custom_post_type_name'], $postId, $display);
        ...
        if(isset($relation['inversedBy']) and null !== $relation['inversedBy']){
            ...
            if(is_array($defaultValues)){
                foreach ($defaultValues as $value){
                    $defaultInversedValues[] = $this->getDefaultValueOfInversed($value, $inversedBy['box'], $inversedBy['field']);
                }
            }

            $field .= '<input type="hidden" name="meta_fields[]" value="'. esc_attr($inversedIdName).RelationCostants::RELATION_KEY.'">';
            $field .= '<input type="hidden" id="inversedBy" name="'. esc_attr($inversedIdName).RelationCostants::RELATION_KEY.'" value="'.esc_attr(implode(',', $defaultInversedValues)).'">';
        }
     ...
     * @return string
     */
    private function getDefaultValueOfInversed($postId, $box, $field)
    {
        $idName = $this->getInversedIdName($box, $field);

        return (get_post_meta($postId, $idName, true) !== null and get_post_meta($postId, $idName, true) !== '' ) ? get_post_meta($postId, $idName, true) : '';
    }
    ...
    {
        return (
            $relationship === MetaBoxFieldRelationshipModel::ONE_TO_MANY_UNI or
            $relationship === MetaBoxFieldRelationshipModel::ONE_TO_MANY_BI or
            $relationship === MetaBoxFieldRelationshipModel::MANY_TO_MANY_UNI or
            $relationship === MetaBoxFieldRelationshipModel::MANY_TO_MANY_BI
        );
    }
    ...
    private function getOptions($postType, $postId = null, $display = null)
    {
        ...
        foreach ($results as $result){
        	...
        	switch ($display){
		        ...
		        case 'radio_checkbox':
			        $label =  $postType . self::LABEL_SEPARATOR . $result->post_title;
			        break;

		        case 'search_box':
		        	$thumbnail = (!empty($result->thumbnail_url)) ? $result->thumbnail_url : '';
			        $label =  $thumbnail . self::LABEL_SEPARATOR.$postType . self::LABEL_SEPARATOR . $result->post_title;
			        break;
