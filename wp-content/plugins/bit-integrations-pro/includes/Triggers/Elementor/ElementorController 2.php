...
final class ElementorController
{
    ...
    public static function getAllFormsFromPostMeta($postMeta)
    {
        ...
        foreach ($postMeta as $widget) {
            foreach ($widget->elements as $elements) {
                foreach ($elements->elements as $element) {
                    if (isset($element->widgetType) && ($element->widgetType == 'form' || $element->widgetType == 'global' )) {
                        $forms[] = $element;
                    }
                }
            }
                ...
                foreach ($elements->elements as $element) {
                    foreach ($element->elements as $subElement) {
                        foreach ($subElement->elements as $subSubElement) {
                            if (isset($subSubElement->widgetType) && ($subSubElement->widgetType == 'form' || $subSubElement->widgetType == 'global')) {
                                $forms[] = $subSubElement;
                            }
                        }
                    }
        ...
        foreach ($postMeta as $widget) {
            foreach ($widget->elements as $elements) {
                if (isset($elements->widgetType) && ($elements->widgetType == 'form' || $elements->widgetType == 'global')) {
                    $forms[] = $elements;
                }
            }
        }
    ...
    private static function getElementorPosts()
    {
        ...
        WHERE $wpdb->posts.post_status = 'publish' AND ($wpdb->posts.post_type = 'post' OR $wpdb->posts.post_type = 'page' OR $wpdb->posts.post_type = 'elementor_library' OR $wpdb->posts.post_type = 'e-landing-page') AND $wpdb->postmeta.meta_key = '_elementor_data'";

        return $wpdb->get_results($query);
    }

// landing page 'e-landing-page'
