...
function awp_convertkit_settings_view( $current_tab ) {
    ...
    <a href="https://sperse.io/go/convertkit" target="_blank"><img src="<?php echo AWP_ASSETS; ?>/images/logos/convertkit.png" width="220" height="50" alt="Convertkit Logo"></a><br/><br/>
    <?php 
...
$form_fields = '';
$app_name= 'convertkit';
$convertkit_form = new AWP_Form_Fields($app_name);

$form_fields = $convertkit_form->awp_wp_text_input(
    array(
        'id'            => "awp_convertkit_display_name",
        'name'          => "awp_convertkit_display_name",
        'value'         => $display_name,
...
$form_fields .= $convertkit_form->awp_wp_text_input(
    array(
        'id'            => "awp_convertkit_api_key",
        'name'          => "awp_convertkit_api_key",
        'value'         => $api_key,
        'placeholder'   => esc_html__( 'Please enter API Key', 'automate_hub' ),
        'label'         =>  esc_html__( 'ConvertKit API Key', 'automate_hub' ),
        'wrapper_class' => 'form-row',
        'show_copy_icon'=>true
...
$form_fields .= $convertkit_form->awp_wp_hidden_input(
    array(
        'name'          => "action",
        'value'         => 'awp_convertkit_save_api_key',
    )
);
...
$convertkit_form->render($form_fields);
...
?>
    ...
    <div class="wrap">
        <form id="form-list" method="post">
            ...
            $platform_obj= new AWP_Platform_Shell_Table('convertkit');
...
add_action( 'admin_post_awp_convertkit_save_api_key', 'awp_save_convertkit_api_key', 10, 0 );

function awp_save_convertkit_api_key() {
    ...
    // Security Check
    if (! wp_verify_nonce( $_POST['_nonce'], 'awp_convertkit_settings' ) ) {
        die( esc_html__( 'Security check Failed', 'automate_hub' ) );
    }
    $api_key = sanitize_text_field( $_POST["awp_convertkit_api_key"] );
    $display_name     = sanitize_text_field( $_POST["awp_convertkit_display_name"] );
    // Save tokens
    $platform_obj= new AWP_Platform_Shell_Table('convertkit');
    $platform_obj->save_platform(['account_name'=>$display_name,'api_key'=>$api_key]);
    AWP_redirect( "admin.php?page=automate_hub&tab=convertkit" );
}

add_action( 'awp_add_js_fields', 'awp_convertkit_js_fields', 10, 1 );

function awp_convertkit_js_fields( $field_data ) {}
    ...
    <script type="text/template" id="convertkit-action-template">
		        <?php
                    ...
                    $app_data=array(
                            'app_slug'=>'convertkit',
                           'app_name'=>'ConvertKit',
                           'app_icon_url'=>AWP_ASSETS.'/images/icons/convertkit.png',
                           'app_icon_alter_text'=>'ConvertKit Icon',
                           'account_select_onchange'=>'getConvertKitLists',
                           'tasks'=>array(
                                        'subscribe'=>array(
                                                            'task_assignments'=>array(

                                                                                    array(
                                                                                        'label'=>'ConvertKit Sequence',
                                                                                        'type'=>'select',
                                                                                        'name'=>"list_id",
...
add_action( 'wp_ajax_awp_get_convertkit_list', 'awp_get_convertkit_list', 10, 0 );
/*
 * Get ConvertKit subscriber lists
 */
function awp_get_convertkit_list() {
...
// Pro
add_action( 'wp_ajax_awp_get_convertkitpro_tags', 'awp_get_convertkitpro_tags', 10, 0 );
/*
 * Get ConvertKit subscriber lists
 */
function awp_get_convertkitpro_tags() {
    ...
    $url = "https://api.convertkit.com/v3/tags?api_key=" . $api_key;
    ...
    $body  = json_decode( $data["body"] );
    $lists = wp_list_pluck( $body->tags, 'name', 'id' );
 ...
 */
function awp_convertkit_save_integration() {
    $params = array();
 ...
 */
function awp_convertkit_send_data( $record, $posted_data ) {
    ...
    if( $task == "subscribe" ) {
        ...
        if( $custom_fields ) {
            ...
            foreach( $holder as $single ) {
                ...
                $data["fields"][strtolower( $single[0] )] = $single[1];
            }
        }
        ...
        if( $tags ) {
            $url = "https://api.convertkit.com/v3/tags/{$tags}/subscribe";
            ...
            $backup_data = $data;
            $backup_data['api_key'] = 'XXXXXXXXXXX';
            $args["body"]=$data;
            awp_add_to_log( $return, $url, $args, $record );
        }
...
function awp_convertkit_resend_data( $log_id,$data,$integration ) {
    ...
    $sequence_id = json_decode( $integration["data"], true );
    $sequence_id = $list_id["field_data"];
    $sequence_id = $sequence_id["listId"];
