...
class CurrencyField extends AbstractFlexibleField implements FlexibleFieldInterface
{
    public function render()
    {
        ...
        $field = '<input type="hidden" name="'. esc_attr($this->getIdName()).'[type]" value="'.AbstractMetaBoxFieldModel::CURRENCY_TYPE.'">';
    ...
    private function getDefaultCurrencyValue()
    {
        ...
        return (isset($data[$key]) and isset($data[$key][$this->elementIndex]) and isset($data[$key][$this->elementIndex]['currency']) ) ? $data[$key][$this->elementIndex]['currency'] : 'USD';
    }
}
