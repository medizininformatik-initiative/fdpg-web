...
add_action( 'awp_custom_script', 'awp_elasticemailpro_custom_script' );

function awp_elasticemailpro_custom_script() {
    wp_enqueue_script( 'awp-elasticemailpro-script', AWP_URL . '/apps/e/elasticemail/elasticemailpro.js', array( 'awp-vuejs' ), '', 1 );
}
...
function awp_elasticemail_settings_view( $current_tab ) {
	...
	<a href="https://sperse.io/go/elasticemail" target="_blank"><img src="<?php echo AWP_ASSETS; ?>/images/logos/elasticemail.png" width="200" height="36" alt="ElasticEmail Logo"></a><br/><br/>
	<?php 
...
$form_fields = '';
$app_name= 'elasticemail';
$elasticemail_form = new AWP_Form_Fields($app_name);

$form_fields = $elasticemail_form->awp_wp_text_input(
    array(
        'id'            => "awp_elasticemail_display_name",
        'name'          => "awp_elasticemail_display_name",
        'value'         => $display_name,
...
$form_fields .= $elasticemail_form->awp_wp_text_input(
    array(
        'id'            => "awp_elasticemail_api_key",
        'name'          => "awp_elasticemail_api_key",
        'value'         => $api_key,
        'placeholder'   => esc_html__( 'Elastic Email API Key', 'automate_hub' ),
        'label'         =>  esc_html__( 'Please enter API Key', 'automate_hub' ),
        'wrapper_class' => 'form-row',
        'show_copy_icon'=>true
...
$form_fields .= $elasticemail_form->awp_wp_hidden_input(
    array(
        'name'          => "action",
        'value'         => 'awp_save_elasticemail_api_key',
    )
);
...
$elasticemail_form->render($form_fields);
...
?>
    ...
    <div class="wrap">
        <form id="form-list" method="post">
            ...
            $platform_obj= new AWP_Platform_Shell_Table('elasticemail');
...
add_action( 'admin_post_awp_save_elasticemail_api_key', 'awp_save_elasticemail_api_key', 10, 0 );

function awp_save_elasticemail_api_key() {
    ...
    // Security Check
    if (! wp_verify_nonce( $_POST['_nonce'], 'awp_elasticemail_settings' ) ) {
        die( esc_html__( 'Security check Failed', 'automate_hub' ) );
    }
    $api_key = sanitize_text_field( $_POST["awp_elasticemail_api_key"] );
    $display_name     = sanitize_text_field( $_POST["awp_elasticemail_display_name"] );
    // Save tokens
    $platform_obj= new AWP_Platform_Shell_Table('elasticemail');
    

    $url = "https://api.elasticemail.com/v2/account/load?apikey={$api_key}";
    $args = array( 'headers' => array( 'Accept' => '*/*' )
    );
    ...
    if( !is_wp_error( $data ) ) {
        ...
        if(isset($body->data->publicaccountid)){
         ...
         $platform_obj->save_platform(['account_name'=>$display_name,'api_key'=>$api_key,'email'=>$public_account_id]);
         AWP_redirect( "admin.php?page=automate_hub&tab=elasticemail" );
         die();
        }
    }
    $platform_obj->save_platform(['account_name'=>$display_name,'api_key'=>"Error Validating API Key"]);
    AWP_redirect( "admin.php?page=automate_hub&tab=elasticemail" );
   
}

add_action( 'awp_add_js_fields', 'awp_elasticemail_js_fields', 10, 1 );

function awp_elasticemail_js_fields( $field_data ) { }
...
?>
    <script type="text/template" id="elasticemail-action-template">
		        <?php
                    ...
                    $app_data=array(
                            'app_slug'=>'elasticemail',
                           'app_name'=>'Elastic Email',
                           'app_icon_url'=>AWP_ASSETS.'/images/icons/elasticemail.png',
                           'app_icon_alter_text'=>'Elastic Email Icon',
                           'account_select_onchange'=>'getElasticEmailList',
                           'tasks'=>array(
                                        'subscribe'=>array(
...
add_action( 'wp_ajax_awp_get_elasticemail_list', 'awp_get_elasticemail_list', 10, 0 );

/*
 * Get Mailchimp subscriber lists
 */
function awp_get_elasticemail_list() {
 ...
 */
function awp_elasticemail_save_integration() {
    $params = array();
 ...
 */
function awp_elasticemail_send_data( $record, $posted_data ) {
    ...
    if( $task == "subscribe" ) {
        ...
        $url="https://api.elasticemail.com/v2/contact/add?apikey=XXXXXXXX";
...
function awp_elasticemail_resend_data( $log_id,$data,$integration ) {
