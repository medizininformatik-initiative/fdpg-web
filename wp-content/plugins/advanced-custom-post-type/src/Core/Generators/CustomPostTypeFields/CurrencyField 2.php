...
class CurrencyField extends AbstractCustomPostTypeField implements MetaFieldInterface
{
    public function render()
    {
        ...
        $icon = 'bx:bx-euro';
        $field = '<input type="hidden" name="'. esc_attr($this->getIdName()).'_type" value="'.CustomPostTypeMetaBoxFieldModel::CURRENCY_TYPE.'">';
        $field .= '<input type="hidden" name="meta_fields[]" value="'.esc_attr($this->getIdName()).'_currency">';
    ...
    {
	    $savedValue = get_post_meta($this->postId, esc_attr($this->getIdName()).'_currency', true);
