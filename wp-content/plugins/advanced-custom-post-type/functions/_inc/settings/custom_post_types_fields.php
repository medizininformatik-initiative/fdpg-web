    ...
    function add_acpt_meta_field($args = [])
    {
        try {
            ...
            if(MetaRepository::existsMetaBoxField([
                ...
                if(isset($args['quick_edit'])){
	                $meta_field_model->setQuickEdit($args['quick_edit']);
                }

	            if(isset($args['is_filterable'])){
		            $meta_field_model->setFilterableInAdmin($args['is_filterable']);
	            }
                ...
                if(isset($args['children'])) {
                    foreach ( $args[ 'children' ] as $index => $child ) {
                        ...
                        // advanced_options
	                    if(isset($child['advanced_options'])){
		                    foreach ($child['advanced_options'] as $index => $option){
			                    ...
			                    $child_field_model->addAdvancedOption($meta_field_advanced_option_model);
		                    }
	                    }

	                    // visibility_conditions
	                    if(isset($child['visibility_conditions'])) {

		                    // data morphing
		                    $visibility_conditions = [];

		                    foreach ( $child[ 'visibility_conditions' ] as $condition ) {
			                    // validate option
			                    $mandatory_keys = [
				                    'type' => [
					                    'required' => true,
		                    ...
		                    VisibilityConditionValidator::validate($child_field_model, $visibility_conditions);
		                    ...
		                    foreach ( $visibility_conditions as $index => $condition ) {
			                    ...
			                    $child_field_model->addVisibilityCondition($visibility_condition_model);
		                    }
	                    }
                ...
                // relations
                if(isset($args['relations']) and isset($args['relations'][0])) {
                    ...
                    $relationship_model = MetaBoxFieldRelationshipModel::hydrateFromArray([
                        ...
                        'relatedEntity' => new RelatedEntityValueObject(MetaTypes::CUSTOM_POST_TYPE, $post_model[0]),
    ...
    function get_acpt_field_object($post_type, $box_name, $field_name)
    {
        try {
            ...
            foreach ($meta_field->getRelations() as $relation){
                ...
                $related_entity = new \stdClass();
	            $related_entity->type = $relation->getRelatedEntity()->getType();
	            $related_entity->value = $relation->getRelatedEntity()->getHumanReadableValue();

	            $related_to->related_entity = $related_entity;
            ...
            foreach ($meta_field->getVisibilityConditions() as $visibilityCondition){
                ...
                if($render_condition_object->type === 'other_fields' and !empty($type['value'])){
                	...
                	if($metaFieldModel !== null){
		                $render_condition_object->meta_field = $metaFieldModel->getName();
	                }
                }
