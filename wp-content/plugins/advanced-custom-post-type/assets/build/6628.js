...
var KeyboardLegendModal = function KeyboardLegendModal(_ref) {
    ...
    var legends = [{
        ...
        keys: [{
            ...
            legend: (0, _useTranslation2.default)("Switch view")
        }, {
            keys: ["Esc"],
        ...
        keys: [{
            ...
            keys: ["▸"],
            legend: (0, _useTranslation2.default)("Navigate the next field (only for Tabular view)")
        }, {
            keys: ["◂"],
            legend: (0, _useTranslation2.default)("Navigate the previous field (only for Tabular view)")
        }, {
            keys: ["Control", "+", "▸"],
...
var _metaHotKeys = __webpack_require__(89407);

var _metaHotKeys2 = _interopRequireDefault(_metaHotKeys);
...
var Meta = function Meta(_ref) {
    ...
    var handleSwitchView = function handleSwitchView(view) {
        localStorage.setItem('META_SWITCH_VIEW_' + id, view);
        dispatch((0, _metaStateActions.switchView)(view));
    };
    ...
    (0, _react.useEffect)(function () {
        ...
        var getDefaultView = function getDefaultView() {
            if (localStorage.getItem('META_SWITCH_VIEW_' + id)) {
                return localStorage.getItem('META_SWITCH_VIEW_' + id);
            }

            return 'list';
        };

        dispatch((0, _metaStateActions.switchView)(getDefaultView()));
    }, [id]);

    // manage key interactions
    var metaHotKeys = new _metaHotKeys2.default(selectedElement, blocks, boxes, fields, values);

    var _metaHotKeys$prevNext = metaHotKeys.prevNextElements(),
        _metaHotKeys$prevNext2 = _slicedToArray(_metaHotKeys$prevNext, 2),
        prev = _metaHotKeys$prevNext2[0],
        next = _metaHotKeys$prevNext2[1];
    ...
    var handleKeyPressEvent = function handleKeyPressEvent(key, e) {
        e.preventDefault();

        var _metaHotKeys$selected = metaHotKeys.selectedElementDetails(),
            isBox = _metaHotKeys$selected.isBox,
            isField = _metaHotKeys$selected.isField,
            hasChildren = _metaHotKeys$selected.hasChildren,
            isOption = _metaHotKeys$selected.isOption,
            isBlock = _metaHotKeys$selected.isBlock,
            isVisibilityCondition = _metaHotKeys$selected.isVisibilityCondition,
            canHaveOptions = _metaHotKeys$selected.canHaveOptions,
            boxId = _metaHotKeys$selected.boxId,
            box = _metaHotKeys$selected.box,
            fieldId = _metaHotKeys$selected.fieldId,
            field = _metaHotKeys$selected.field,
            optionId = _metaHotKeys$selected.optionId,
            visibilityConditionId = _metaHotKeys$selected.visibilityConditionId,
            block = _metaHotKeys$selected.block,
            blockId = _metaHotKeys$selected.blockId,
            isSaved = _metaHotKeys$selected.isSaved;
        ...
        switch (key) {
            ...
            case "up":
                ...
                var prevField = fields.filter(function (f) {
                    return f.props.id === prev;
                });

                if (prevField.length === 1) {
                    dispatch((0, _metaStateActions.setCurrentFieldTab)(prevField[0].props.boxId, prevField[0].props.id, prevField[0].props.parentId ? prevField[0].props.parentId : null, prevField[0].props.blockId ? prevField[0].props.blockId : null));

                    if (view === 'tabular' && !prevField[0].props.parentId && !prevField[0].props.blockId) {
                        (0, _scroll.scrollToId)(prevField[0].props.boxId, -230);
                    }
                }
            ...
            case "down":
                ...
                var nextField = fields.filter(function (f) {
                    return f.props.id === next;
                });

                if (nextField.length === 1) {
                    dispatch((0, _metaStateActions.setCurrentFieldTab)(nextField[0].props.boxId, nextField[0].props.id, nextField[0].props.parentId ? nextField[0].props.parentId : null, nextField[0].props.blockId ? nextField[0].props.blockId : null));

                    if (view === 'tabular' && !nextField[0].props.parentId && !nextField[0].props.blockId) {
                        (0, _scroll.scrollToId)(nextField[0].props.boxId, -230);
                    }
                }
            ...
            case 'left':
                if (view === 'tabular' && isField) {

                    var currentFieldIndex = fields.findIndex(function (f) {
                        return f.props.id === fieldId;
                    });

                    if (typeof fields[currentFieldIndex - 1] !== 'undefined') {
                        dispatch((0, _metaStateActions.selectElement)(fields[currentFieldIndex - 1].props.id));
                        dispatch((0, _metaStateActions.setCurrentFieldTab)(fields[currentFieldIndex - 1].props.boxId, fields[currentFieldIndex - 1].props.id, fields[currentFieldIndex - 1].props.parentId ? fields[currentFieldIndex - 1].props.parentId : null, fields[currentFieldIndex - 1].props.blockId ? fields[currentFieldIndex - 1].props.blockId : null));
                    }
                }
            ...
            case 'right':
                if (view === 'tabular' && isField) {
                    ...
                    var _currentFieldIndex = fields.findIndex(function (f) {
                        return f.props.id === fieldId;
                    });

                    if (typeof fields[_currentFieldIndex + 1] !== 'undefined') {
                        dispatch((0, _metaStateActions.selectElement)(fields[_currentFieldIndex + 1].props.id));
                        dispatch((0, _metaStateActions.setCurrentFieldTab)(fields[_currentFieldIndex + 1].props.boxId, fields[_currentFieldIndex + 1].props.id, fields[_currentFieldIndex + 1].props.parentId ? fields[_currentFieldIndex + 1].props.parentId : null, fields[_currentFieldIndex + 1].props.blockId ? fields[_currentFieldIndex + 1].props.blockId : null));
                    }
                }
            ...
            case "command+shift+V":
                var newView = view === 'tabular' ? 'list' : 'tabular';
 ...
 * @param values
 */
function MetaHotKeys(selectedElement, blocks, boxes, fields, values) {
    var _this = this;

    _classCallCheck(this, MetaHotKeys);

    this.keys = function () {
        return ['esc', 'escape', 'tab', 'up', 'down', 'left', 'right', 'control+left', 'command+left', 'control+right', 'command+right', 'control+A', 'command+A', 'control+B', 'command+B', 'control+C', 'command+C', 'control+D', 'command+D', 'control+F', 'command+F', 'control+L', 'command+L', 'control+O', 'command+O', 'control+R', 'command+R', 'control+S', 'command+S', 'control+del', 'command+del', 'control+shift+A', 'command+shift+A', 'control+shift+B', 'command+shift+B', 'control+shift+R', 'command+shift+R', 'control+V', 'command+V', 'control+shift+del', 'command+shift+del', 'control+shift+V', 'command+shift+V'];
    };

    this.keysMap = function () {
        return _this.keys().join(',');
    };

    this.selectedElementDetails = function () {
        ...
        if (_this.selectedElement) {
            _this.values.forEach(function (box) {
                if (box.id === _this.selectedElement) {

                    var selectedBox = _this.boxes.filter(function (b) {
                        return b.props.id === _this.selectedElement;
                    })[0];
                ...
                box.fields && box.fields.forEach(function (field) {
                    ...
                    if (field.id === _this.selectedElement) {

                        var selectedField = _this.fields.filter(function (f) {
                            return f.props.id === _this.selectedElement;
                        })[0];
                    ...
                    field.visibilityConditions && field.visibilityConditions.forEach(function (condition) {
                        if (condition.id === _this.selectedElement) {
                            object = {
                                isBox: false,
                                isField: false,
                                hasChildren: false,
                    ...
                    canHaveOptions && field.options && field.options.forEach(function (option) {
                        if (option.id === _this.selectedElement) {
                            object = {
                                isBox: false,
                                isField: false,
                                hasChildren: false,
                    ...
                    field.blocks && field.blocks.forEach(function (block) {
                        ...
                        if (block.id === _this.selectedElement) {

                            var selectedBlock = _this.blocks.filter(function (f) {
                                return f.props.id === _this.selectedElement;
                            })[0];
                        ...
                        block.fields && block.fields.forEach(function (nestedField) {
                            ...
                            if (nestedField.id === _this.selectedElement) {
                                var _selectedField = _this.fields.filter(function (f) {
                                    return f.props.id === _this.selectedElement;
                                })[0];
                    ...
                    field.children && field.children.forEach(function (child) {
                        ...
                        if (child.id === _this.selectedElement) {

                            var _selectedField2 = _this.fields.filter(function (f) {
                                return f.props.id === _this.selectedElement;
                            })[0];
    ...
    this.prevNextElements = function () {
        ...
        if (_this.selectedElement === null) {
            ...
            var index = ids.indexOf(_this.selectedElement);
...
/**
 *
 * @return {{canHaveOptions: boolean, isBlock: boolean, visibilityConditionId: null, hasChildren: boolean, isField: boolean, isOption: boolean, box: null, isVisibilityCondition: boolean, parentFieldId: null, blockId: null, field: null, isSaved: boolean, optionId: null, block: null, isBox: boolean, isChild: boolean, boxId: null, fieldId: null}}
 */
