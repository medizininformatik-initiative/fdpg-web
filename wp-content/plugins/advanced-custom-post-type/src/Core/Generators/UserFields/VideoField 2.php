...
class VideoField extends AbstractUserField implements MetaFieldInterface
{
    public function render()
    {
        ...
        $icon = 'bx:bx-video';
        $field = '<input type="hidden" name="'. esc_attr($this->getIdName()).'_type" value="'.UserMetaBoxFieldModel::VIDEO_TYPE.'">';
        ...
        $field .= '<input readonly '.$this->required().' id="'.esc_attr($this->getIdName()).'" name="'. esc_attr($this->getIdName()).'" type="text" class="regular-text" value="' .esc_attr($this->getDefaultValue()) .'">';
