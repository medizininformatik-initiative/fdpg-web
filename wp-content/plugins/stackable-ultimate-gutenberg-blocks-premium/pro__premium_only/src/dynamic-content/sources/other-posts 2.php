...
class Other_Posts {
    ...
    public static function initialize_other_fields( $output, $id ) {
		...
		if ( empty( $meta_keys ) || ! is_array( $meta_keys ) ) {
			$meta_keys = $wpdb->get_results( 'select distinct meta_key from ' . $wpdb->prefix . 'postmeta where meta_key not like "\_%"' );
			...
			update_option( 'stackable_dynamic_content_meta_keys_frontend', $meta_keys );
		}
