			...
			switch ($meta_field_model_type){
				...
				case OptionPageMetaBoxFieldModel::ADDRESS_TYPE:
					...
					if($saved_field_value !== $value){
						...
						if(!update_option($meta_field_model->getDbName(), Sanitizer::sanitizePostTypeRawData(OptionPageMetaBoxFieldModel::ADDRESS_TYPE, $value))){
							return false;
						}
						...
						if($coordinates){
							if(!update_option( $meta_field_model->getDbName().'_lat', Sanitizer::sanitizePostTypeRawData(OptionPageMetaBoxFieldModel::TEXT_TYPE, $coordinates['lat']))){
								return false;
							}

							if(!update_option($meta_field_model->getDbName().'_lng', Sanitizer::sanitizePostTypeRawData(OptionPageMetaBoxFieldModel::TEXT_TYPE, $coordinates['lng']))){
								return false;
							}
						}
					}
				...
				case OptionPageMetaBoxFieldModel::CURRENCY_TYPE:
					...
					if($saved_field_value !== $amount){
						if(!update_option($meta_field_model->getDbName(), Sanitizer::sanitizePostTypeRawData(OptionPageMetaBoxFieldModel::CURRENCY_TYPE, $amount))){
							return false;
						}
					}

					if($saved_field_unit !== $unit){
						if(!update_option($meta_field_model->getDbName().'_currency', Sanitizer::sanitizePostTypeRawData(OptionPageMetaBoxFieldModel::TEXT_TYPE, $unit))){
							return false;
						}
					}
				...
				case OptionPageMetaBoxFieldModel::DATE_RANGE_TYPE:
					...
					$value = implode(" - ", $value);
					if(!update_option($meta_field_model->getDbName(), Sanitizer::sanitizePostTypeRawData($meta_field_model_type, $value))){
						return false;
					}
				...
				case OptionPageMetaBoxFieldModel::FILE_TYPE:
					...
					if($saved_field_value !== $url){
						if(!update_option($meta_field_model->getDbName(), Sanitizer::sanitizePostTypeRawData($meta_field_model_type, $url))){
							return false;
						}
					}

					if($saved_field_label !== $label){
						if(!update_option($meta_field_model->getDbName().'_label', Sanitizer::sanitizePostTypeRawData(OptionPageMetaBoxFieldModel::TEXT_TYPE, $label))){
							return false;
						}
					}
				...
				case OptionPageMetaBoxFieldModel::LENGTH_TYPE:
					...
					if($saved_field_value !== $length){
						if(!update_option($meta_field_model->getDbName(), Sanitizer::sanitizePostTypeRawData(OptionPageMetaBoxFieldModel::WEIGHT_TYPE, $length))){
							return false;
						}
					}

					if($saved_field_unit !== $unit){
						if(!update_option($meta_field_model->getDbName().'_length', Sanitizer::sanitizePostTypeRawData(OptionPageMetaBoxFieldModel::TEXT_TYPE, $unit))){
							return false;
						}
					}
				...
				case OptionPageMetaBoxFieldModel::WEIGHT_TYPE:
					...
					if($saved_field_value !== $weight){
						if(!update_option($meta_field_model->getDbName(), Sanitizer::sanitizePostTypeRawData(OptionPageMetaBoxFieldModel::WEIGHT_TYPE, $weight))){
							return false;
						}
					}

					if($saved_field_unit !== $unit){
						if(!update_option($meta_field_model->getDbName().'_weight', Sanitizer::sanitizePostTypeRawData(OptionPageMetaBoxFieldModel::TEXT_TYPE, $unit))){
							return false;
						}
					}
...
if( !function_exists( 'add_acpt_option_page_meta_block_field_row_value' ) ){
	...
	function add_acpt_option_page_meta_block_field_row_value(array $args = [])
	{
		try {
			...
			if(!update_option($meta_field_model->getDbName(), Sanitizer::sanitizePostTypeRawData(OptionPageMetaBoxFieldModel::FLEXIBLE_CONTENT_TYPE, $values))){
				return false;
			}
		...
		try {
			...
			switch ($meta_field_model_type){
				...
				case OptionPageMetaBoxFieldModel::LIST_TYPE:
					...
					if(!update_option($meta_field_model->getDbName(), Sanitizer::sanitizePostTypeRawData(OptionPageMetaBoxFieldModel::LIST_TYPE, $values))){
						return false;
					}
					...
					case OptionPageMetaBoxFieldModel::REPEATER_TYPE:
						...
						if(!update_option($meta_field_model->getDbName(), Sanitizer::sanitizePostTypeRawData(OptionPageMetaBoxFieldModel::REPEATER_TYPE, $values))){
							return false;
						}
...
if( !function_exists('delete_acpt_option_page_meta_field_row_value') )
{
	function delete_acpt_option_page_meta_field_row_value(array $args = [])
	{
		try {
			...
			if(!update_option($meta_field_model->getDbName(), Sanitizer::sanitizePostTypeRawData(OptionPageMetaBoxFieldModel::LIST_TYPE, array_values($saved_field_value)))){
				return false;
			}
			...
			if(empty($acpt_field)){
				delete_acpt_option_page_meta_field($args['option_page'], $args['box_name'], $args['field_name']);
			}
			...
			switch ($meta_field_model_type){
				...
				case OptionPageMetaBoxFieldModel::LIST_TYPE:
					...
					if($saved_field_value[$index] !== $value){
						...
						if(!update_option($meta_field_model->getDbName(), Sanitizer::sanitizePostTypeRawData(OptionPageMetaBoxFieldModel::LIST_TYPE, $saved_field_value))){
							return false;
						}
					}
...
if( !function_exists('delete_acpt_option_page_meta_block_field_row_value')){
	...
	function delete_acpt_option_page_meta_block_field_row_value(array $args = [])
	{
		try {
			...
			if(!update_option($meta_field_model->getDbName(), Sanitizer::sanitizePostTypeRawData(OptionPageMetaBoxFieldModel::FLEXIBLE_CONTENT_TYPE, $saved_field_value))){
				return false;
			}
