		...
		foreach ($optionPageModel->getMetaBoxes() as $boxModel) {
			foreach ($boxModel->getFields() as $fieldModel) {
				...
				if(isset($data[$idName])){
					...
					update_option($idName, Sanitizer::sanitizeRawData($fieldModel->getType(), $value));
					...
					foreach ($extras as $extra){
						if(isset($data[$idName.'_'.$extra])){
							update_option($idName.'_'.$extra, Sanitizer::sanitizeRawData(OptionPageMetaBoxFieldModel::TEXT_TYPE, $data[$idName.'_'.$extra] ) );
						}
					}
				}
