...
class AWP_Sendgrid extends appfactory
{
    ...
    public function settings_view($current_tab)
    {
        ...
        <div class="platformheader">
            <a href="https://sperse.io/go/sendgrid" target="_blank"><img src="<?=AWP_ASSETS;?>/images/logos/sendgrid.png" width="275" height="50" alt="Sendgrid Logo"></a><br /><br />
            <?php 
...
$form_fields = '';
$app_name= 'sendgrid';
$sendgrid_form = new AWP_Form_Fields($app_name);

$form_fields = $sendgrid_form->awp_wp_text_input(
    array(
        'id'            => "awp_sendgrid_display_name",
        'name'          => "awp_sendgrid_display_name",
        'value'         => $display_name,
...
$form_fields .= $sendgrid_form->awp_wp_text_input(
    array(
        'id'            => "awp_sendgrid_api_token",
        'name'          => "awp_sendgrid_api_token",
        'value'         => $api_key,
        'placeholder'   => esc_html__( 'Enter your Sendgrid API Key', 'automate_hub' ),
        'label'         =>  esc_html__( 'Sendgrid API Key', 'automate_hub' ),
        'wrapper_class' => 'form-row',
        'show_copy_icon'=>true
...
$form_fields .= $sendgrid_form->awp_wp_hidden_input(
    array(
        'name'          => "action",
        'value'         => 'awp_sendgrid_save_api_token',
    )
);
...
?>
        ...
        <div class="wrap">
                <form id="form-list" method="post">
                    ...
                    $platform_obj= new AWP_Platform_Shell_Table('sendgrid');
    ...
    public function awp_save_sendgrid_api_token()
    {
        if (!current_user_can('administrator')) {
            die(esc_html__('You are not allowed to save changes!', 'automate_hub'));
        }
        // Security Check
        if (!wp_verify_nonce($_POST['_nonce'], 'awp_sendgrid_settings')) {
            die(esc_html__('Security check Failed', 'automate_hub'));
        }
        $api_token = sanitize_text_field($_POST["awp_sendgrid_api_token"]);
        $display_name     = sanitize_text_field( $_POST["awp_sendgrid_display_name"] );
        // Save tokens
        $platform_obj= new AWP_Platform_Shell_Table('sendgrid');
        $platform_obj->save_platform(['account_name'=>$display_name,'api_key'=>$api_token]);

        AWP_redirect("admin.php?page=automate_hub&tab=sendgrid");
    }

    public function awp_sendgrid_js_fields($field_data)
    {
    }

    public function load_custom_script()
    {
        wp_enqueue_script('awp-sendgrid-script', AWP_URL . '/apps/s/sendgrid/sendgrid.js', array('awp-vuejs'), '', 1);
    }
    ...
    <script type="text/template" id="sendgrid-action-template">
                <?php
                    ...
                    $app_data=array(
                            'app_slug'=>'sendgrid',
                           'app_name'=>'Sendgrid',
                           'app_icon_url'=>AWP_ASSETS.'/images/icons/sendgrid.png',
                           'app_icon_alter_text'=>'Sendgrid Icon',
                           'account_select_onchange'=>'getSendgridList',
                           'tasks'=>array(
                                        'subscribe'=>array(
...
<?php
     ...
     */
    public function awp_get_sendgrid_list()
    {
        if (!current_user_can('administrator')) {
            die(esc_html__('You are not allowed to save changes!', 'automate_hub'));
        }
 ...
 */
function awp_sendgrid_save_integration() {
    Appfactory::save_integration();
}

/*
 * Handles sending data to sendgrid API
 */
function awp_sendgrid_send_data($record, $posted_data)
{
    ...
    $decoded_data = AWP_Sendgrid::decode_data($record, $posted_data);
...
function awp_sendgrid_resend_data($log_id, $data, $integration)
{
