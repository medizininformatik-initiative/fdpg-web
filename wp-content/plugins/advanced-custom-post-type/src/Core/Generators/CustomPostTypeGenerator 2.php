...
class CustomPostTypeGenerator extends AbstractGenerator
{
    ...
    public function saveCustomPostType($postId)
    {
	    ...
	    if (!Nonce::verify($_POST[Nonce::ACPT_NONCE])) {
		    return;
	    }
                ...
                if (!empty($_FILES[$key])) {
                    ...
                    if(isset($_POST[$key])){
                        ...
                        if(Strings::contains(RelationCostants::RELATION_KEY, $key)){
                            ...
                            update_post_meta($postId, $key, Sanitizer::sanitizePostTypeRawData($type, $value));
                        }
                    } else {
    ...
    /**
     * This function handles the post relationship complex logics
     *
     * @param int $postId
     * @param string $key
     *
    ...
    private function handleRelations($postId, $key)
    {
        ...
        if('' !== $_POST[$key]){
            ...
            foreach ($inversedIds as $inversedId){
                ...
                $customPostType = MetaRepository::getMetaFieldByName([
                    'belongsTo' => MetaTypes::CUSTOM_POST_TYPE,
                    'find' => get_post_type($inversedId),
                    'boxName' => $box,
                    'fieldName' => $field,
                ]);

                if($customPostType !== null){
	                $relationships = $customPostType->getRelations();
	                ...
	                if(!empty($relationships)){
		                ...
		                if(true === $relationship->isMany()){
			                ...
			                update_post_meta($inversedId, $plainKey, Sanitizer::sanitizePostTypeRawData(CustomPostTypeMetaBoxFieldModel::POST_TYPE, $previousValues));

		                } else {
			                $this->deleteOrUpdateOtherPostMeta($relationship->getInversedBy()->getDbName(), $postId, $inversedId);
			                update_post_meta($inversedId, $plainKey, Sanitizer::sanitizePostTypeRawData(CustomPostTypeMetaBoxFieldModel::POST_TYPE, $postId));
		                }
	                }
                }
            }
     ...
     *
     * This function deletes or updates the cpts metas with post_id = $postId which have $inversedId as value
     *
     * @param $key
     * @param $postId
     * @param $inversedId
    ...
    private function deleteAllInversedKeys($key, $postId)
    {
        ...
        $results = ACPT_DB::getResults($baseQuery, [$key]);
        ...
        foreach ($results as $result){
            ...
            if(is_serialized($result->meta_value)){
                ...
                $value = unserialize($result->meta_value);
                $postIdMatch = array_search($postId, $value);
